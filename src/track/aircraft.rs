...
impl Default for CircularBuffer_8_F64 {
    fn default() -> Self { Self{ buffer:[0.0; 8], idx:0 }}
}
...
impl Aircraft {
    ...
    pub fn update_position(&mut self, mm:&mut ModeSMessage, now_ms:u128) -> Result<(), &'static str> {
        ...
        if let Some((_, _, cpr_odd, _, cpr_type)) = mm.raw_cpr {
            ...
            let cpr_same_type = if let (Some((even, _, _, _)), Some((odd, _, _, _))) = (self.cpr_even.get_opt(), self.cpr_odd.get_opt()) {
                odd == even
            } else { false };

            if self.cpr_odd.is_valid() && self.cpr_even.is_valid() && self.cpr_odd.same_source(&self.cpr_even) && 
                cpr_same_type && self.cpr_odd.time_between(&self.cpr_even) <= max_elapsed {

                location_result = do_global_cpr(self, mm, now_ms, &mut new_lat, &mut new_lon, &mut new_nuc);
                ...
                if location_result == -2 {
                    ...
                    self.position.combine_validity(&self.cpr_even, &self.cpr_odd);
                }
            }
            ...
            if location_result == -1 {
                location_result = do_local_cpr(self, mm, now_ms, &mut new_lat, &mut new_lon, &mut new_nuc);

                if location_result >= 0 {

                    cpr_relative = true;    // true means local

                    if cpr_odd { self.position.copy_validity_from(&self.cpr_odd);  } 
                    else       { self.position.copy_validity_from(&self.cpr_even); }
                }
            }
    ...
    pub fn speed_check(&self, lat:f64, lon:f64, now_ms:u128, surface:bool) -> bool
    {
        ...
        let mut speed:u32 = self.speed.get_if_valid().map(|x| *x)
        ...
        let range:f64 = (if surface { 0.1e3 } else { 0.5e3 }) + ((elapsed as f64 + 1000.0) / 1000.0) * (speed as f64 * 1852.0 / 3600.0);
...
fn do_global_cpr(a:&mut Aircraft, mm:&ModeSMessage, now_ms:u128, lat:&mut f64, lon:&mut f64, nuc:&mut u32) -> i32 {
    ...
    *nuc = std::cmp::min(*even_nuc, *odd_nuc); // worst of the two positions
    ...
    // check speed limit
    if a.position.is_valid() && a.position.get_opt().unwrap().2 >= *nuc && !a.speed_check(*lat, *lon, now_ms, surface) {
        return -2;
    }
...
fn do_local_cpr(a:&mut Aircraft, mm:&ModeSMessage, now_ms:u128, lat:&mut f64, lon:&mut f64, nuc:&mut u32) -> i32 {
    // relative CPR
    // find reference location
    ...
    *nuc = cpr_nucp;
    ...
    let (reflat, reflon, range_limit) = if a.position.is_valid_with_constraints(now_ms, 50000, DataSource::Invalid) {
