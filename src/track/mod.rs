...
use crate::{MODEAC_MSG_FLAG, MODEAC_MSG_MODEA_ONLY, MODEAC_MSG_MODEC_HIT, MODEAC_MSG_MODEA_HIT, MODEAC_MSG_MODEC_OLD};
...
impl<T:Default> DataValidityBox<T> {
	...
	pub fn get_opt(&self) -> Option<&T> { self.opt_contents.as_ref()         }
	pub fn direct_set(&mut self, v:T)   { self.opt_contents = Some(v)        }
	pub fn is_valid(&self) -> bool      { self.source != DataSource::Invalid }
	pub fn direct_set_source(&mut self, src:DataSource) { self.source = src; }
	pub fn get_if_valid(&self) -> Option<&T> { if self.is_valid() { self.get_opt() } else { None } }

	pub fn same_source<U: Default>(&self, other:&DataValidityBox<U>)  -> bool  { self.source == other.source }
	pub fn time_between<U: Default>(&self, other:&DataValidityBox<U>) -> u128  { 
		if self.updated > other.updated { self.updated - other.updated } else { other.updated - self.updated } 
	}
	pub fn is_valid_with_constraints(&self, now_ms:u128, max_age_ms:u128, min_source:DataSource) -> bool {
		self.is_valid() && self.source >= min_source && !((self.updated < now_ms) && (now_ms - self.updated > max_age_ms))
	}

	pub fn data_age(&self, now_ms:u128) -> u128 {
		if self.source == DataSource::Invalid { std::u128::MAX }
		else if self.updated > now_ms         { 0 }
		else                                  { now_ms - self.updated }
	}
	...
	// track.c:115
	pub fn update<U, V: Fn(&mut T) -> U>(&mut self, mm:&ModeSMessage, now_ms:u128, f:V) -> Option<U> {
	    ...
	    if self.opt_contents.is_none() { self.opt_contents = Some(T::default()) }
		
	    self.opt_contents.as_mut().map(f)
	}

	pub fn opt_fn_update<U, V: Fn(&U) -> T>(&mut self, mm:&ModeSMessage, now_ms:u128, opt_input:&Option<U>, f:V) {
	...
	// track.c:128
	pub fn combine_validity<U: Default, V: Default>(&mut self, from1:&DataValidityBox<U>, from2:&DataValidityBox<V>) {
	...
	pub fn compare_validity<U: Default>(&self, rhs:&DataValidityBox<U>, now_ms:u128) -> i8 {
	    if      now_ms < self.stale && self.source > rhs.source {  1 }
	    else if now_ms < rhs.stale  && self.source < rhs.source { -1 }
	    else if self.updated > rhs.updated                      {  1 }
	    else if self.updated < rhs.updated 						{ -1 }
	    else 													{  0 }
	}
...
fn greatcircle(lat0_deg:f64, lon0_deg:f64, lat1_deg:f64, lon1_deg:f64) -> f64
{
    ...
    if dlat < 0.001 && dlon < 0.001 {
        let a = (dlat/2.0).sin() * (dlat/2.0).sin() + lat0.cos() * lat1.cos() * (dlon/2.0).sin() * (dlon/2.0).sin();
...
pub fn update_from_message(mm: &mut ModeSMessage) {
    ...
    if let Ok(mut aircraft_list) = AIRCRAFT_LIST.lock() {
    	...
    	let mut a:&mut aircraft::Aircraft = aircraft_list.iter_mut().find(|a| a.addr.0 == mm.addr).unwrap();

    	if mm.signal_level > 0.0       { a.signal_level.push(mm.signal_level); }
	    ...
	    a.addr = match (a.addr.clone(), mm.addrtype.clone()) {
	    	...
	    	((addr, old_typ), Some(new_typ)) => if new_typ < old_typ { (addr, new_typ) } else { (addr, old_typ) }
	    };
	    ...
	    a.gnss_delta.opt_fn_update(&mm, now_ms, &mm.gnss_delta, |x| *x);

	    match mm.heading {
	    	Some((mm_hdg, HeadingSource::True))     => { a.heading.update(         &mm, now_ms, |hdg| *hdg = mm_hdg );          },
	    	Some((mm_hdg, HeadingSource::Magnetic)) => { a.heading_magnetic.update(&mm, now_ms, |hdg| *hdg = mm_hdg ); },
	    	_ => ()
	    }

	    match mm.speed {
	    	Some((mm_spd, SpeedSource::GroundSpeed)) => { a.speed.update(    &mm, now_ms, |spd| *spd = mm_spd); },
	    	Some((mm_spd, SpeedSource::IAS))         => { a.speed_ias.update(&mm, now_ms, |spd| *spd = mm_spd); },
	    	Some((mm_spd, SpeedSource::TAS))         => { a.speed_tas.update(&mm, now_ms, |spd| *spd = mm_spd); },
	    	_ => ()
	    }
	    ...
	    a.vert_rate.opt_fn_update(&mm, now_ms, &mm.vert_rate, |x| x.clone());
	    a.category.opt_fn_update( &mm, now_ms, &mm.category,  |x| *x);
	    a.airground.opt_fn_update(&mm, now_ms, &mm.airground, |x| *x);
	    a.callsign.opt_fn_update( &mm, now_ms, &mm.callsign,  |x| x.clone());

	    if let Some((mm_cpr_lat, mm_cpr_lon, mm_cpr_odd, mm_cpr_nucp, mm_cpr_type)) = mm.raw_cpr {
	    	if mm_cpr_odd { a.cpr_odd.update( &mm, now_ms, |x| *x = (mm_cpr_type, mm_cpr_lat, mm_cpr_lon, mm_cpr_nucp)); } 
	    	else          { a.cpr_even.update(&mm, now_ms, |x| *x = (mm_cpr_type, mm_cpr_lat, mm_cpr_lon, mm_cpr_nucp)); }
	    }

	    // Now handle derived data

	    // derive GNSS if we have baro + delta
	    if a.altitude.compare_validity(&a.altitude_gnss, now_ms) > 0 && a.gnss_delta.compare_validity(&a.altitude_gnss, now_ms) > 0 {
	        // Baro and delta are both more recent than GNSS, derive GNSS from baro + delta
	        if let (Some((baro_alt, _)), Some(gnss_delta)) = (a.altitude.get_opt(), a.gnss_delta.get_opt()) {
